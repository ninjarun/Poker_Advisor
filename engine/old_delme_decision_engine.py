...
class DecisionEngine:
    ...
    def _preflop_chart_decision(self, hand_code: str, position: str, amount_to_call: float) -> Tuple[str, int, str]:
        ...
        if amount_to_call > 0 and hand_code in three_bet_bluffs:
            if self.randomness_engine.mixed_action("RAISE", "CALL", 0.25) == "RAISE":
                return "RAISE", 4, "Polarized 3-bet bluff: blocker/suited connector mixed at 25% versus TAG opens."
            return "CALL", 4, "Mixed 3-bet bluff candidate not selected; continue with playable blocker/draw equity."
